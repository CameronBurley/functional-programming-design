...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    b.legalNeighbors.map((neighbor, move) => (neighbor, move :: history)).to(LazyList)
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors.filter((block, _) => !explored(block))
  ...
  def from(initial: LazyList[(Block, List[Move])], explored: Set[Block]): LazyList[(Block, List[Move])] = 
    initial match {
      ...
      case (block, history) #:: tail =>
        ...
        (block, history) +: from(tail ++ newNeighbors, explored + block)
    }
   ...
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(LazyList((startBlock, List.empty)), Set.empty)
